...
export default {
  ...
  methods: {
	...
	this.ruleForm.wenzhangneirong = this.ruleForm.wenzhangneirong.replace(reg,'../../../ssm2b4x5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `hanfuzhishi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.hanfuzhishiCrossAddOrUpdateFlag = false;
                  this.parent.search();
